...
Вложенный запрос – это запрос, который находится внутри другого SQL запроса. Подзапрос всегда заключается в круглые скобки и обычно выполняется перед основным запросом.

Возможны три варианта:

- Подзапрос возвращает одно единственное значение (одна колонка и одна строка). Это скалярный подзапрос. Пример: задача 1. Чаще всего он используется в условиях ограничений выборки с помощью операторов сравнения (=, <>, >, <).
- Подзапрос возвращает список значений (таблица с одной колонкой и несколькими строками). Пример: задача 2. Его нельзя просто использовать с операторами сравнения, но можно использовать операторы IN, ALL, ANY
- Подзапрос возвращает таблицу (много колонок, любое количество строк). В этом случае оператор IN работает с кортежами. Например, вывести бронирования, в которых цена жилья на момент брони равна текущей стоимости жилья 

```sql
SELECT * FROM Reservations
    WHERE (room_id, price) IN (SELECT id, price FROM Rooms);
```
Решение без подзапроса более громоздкое:
...
Некоррелированные подзапросы можно выполнять автономно от основного запроса и можно посмотреть, что они возвращают перед тем, как их результат будет использоваться в основном запросе. Коррелированные же подзапросы ссылаются на один или несколько столбцов основного запроса.

Коррелированный подзапрос выполняется не один раз перед выполнением основного запроса, а для каждой строки, которая может быть включена в окончательный результат.
...
[курс по SQL](https://sql-academy.org/ru/guide)
