...
> План выполнения. Оптимизатор БД обычно строит идентичный план в обоих синтаксисах

CTE имеет преимущества:

- Можно ссылаться на CTE несколько раз в одном запросе, а подзапрос нужно дублировать
- Читаемость выше - логика разделена на шаги

Внешний запрос получает уже "готовую" таблицу с колонкой `drnk` и может применить к ней `WHERE`.

Прямое условие `WHERE DENSE_RANK() <= 2` в одном запросе запрещено стандартом SQL, потому что оконные функции вычисляются после фильтрации `WHERE`. Подзапрос решает эту проблему, сдвигая вычисление на шаг раньше.
